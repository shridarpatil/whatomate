...
<template>
  ...
  <div v-else-if="isUser" class="flex justify-end">
    <div class="max-w-[85%]">
      <div
        class="bg-[#005c4b] light:bg-[#d9fdd3] rounded-lg rounded-tr-none shadow-sm p-3"
      >
        <p class="text-sm text-gray-200 light:text-gray-800">
          {{ message.content }}
        </p>
        <p
          class="text-[10px] text-gray-400 light:text-gray-500 text-right mt-1 flex items-center justify-end gap-1"
        >
          {{ formattedTime }}
          <svg
            class="h-4 w-4 text-[#53bdeb]"
            viewBox="0 0 24 24"
            fill="currentColor"
          >
            <path
  ...
  <div v-else-if="isSystem" class="flex justify-center">
    <div
      class="bg-amber-100 dark:bg-amber-900/30 text-xs text-amber-700 dark:text-amber-400 px-3 py-1.5 rounded-lg flex items-center gap-1.5"
    >
      <Info class="h-3 w-3" />
  ...
  <div v-else-if="isDebug" class="flex justify-center">
    <div
      class="bg-purple-100 dark:bg-purple-900/30 text-xs text-purple-700 dark:text-purple-400 px-3 py-1.5 rounded-lg flex items-center gap-1.5 max-w-[90%]"
    >
