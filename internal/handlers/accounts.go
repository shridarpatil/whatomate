...
func (a *App) TestAccountConnection(r *fastglue.Request) error {
	...
	req, err := http.NewRequestWithContext(r.RequestCtx, http.MethodGet, url, nil)
	if err != nil {
	...
	if (messagingLimitTier == nil || messagingLimitTier == "") && account.BusinessID != "" {
		...
		wabaReq, err := http.NewRequestWithContext(r.RequestCtx, http.MethodGet, wabaURL, nil)
		if err == nil {
			...
			if err == nil {
				...
				if wabaResp.StatusCode == http.StatusOK {
