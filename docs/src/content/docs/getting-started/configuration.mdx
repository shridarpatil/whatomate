---
title: Configuration
description: Complete configuration reference for Ritam Bharat
---

import { Aside } from '@astrojs/starlight/components';

Ritam Bharat uses a TOML configuration file for all settings. By default, it looks for `config.toml` in the current directory; pass `-config /path/to/config.toml` to override.
...
[database]
host = "localhost"            # Use "db" inside Docker Compose; keep localhost for local installs
port = 5432
user = "ritam_bharat"
...
[redis]
host = "localhost"            # Use "redis" inside Docker Compose; keep localhost for local installs
port = 6379
...
|---|---|---|
| `name` | no | Display name. Defaults to `"Ritam Bharat"`. |
...
| `base_path` | Leave empty unless serving at a sub-path (`https://app.example.com/ritam-bharat`) behind a proxy. |
...
- Backend: `make run-migrate` (or `./ritam-bharat server -migrate`)
