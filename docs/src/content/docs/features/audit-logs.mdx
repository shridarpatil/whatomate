...
| `chatbot_flow` | Chatbot flow create / update / delete |
| `contact` | Contact create / update / delete |
| `canned_response` | Saved reply create / update / delete |
| `ivr_flow` | IVR flow definitions |
| `settings.general` | General org settings changes |
| `settings.calling` | Voice calling settings changes |
| `settings.notification` | Notification preference changes |
...
A set of fields is excluded from diffs to keep the audit log readable and to avoid logging anything sensitive or noisy. The lists below are **illustrative** — a subset of the full skip list (`skipFields` in `internal/audit/audit.go`), not an exhaustive enumeration:
