...
<CardGrid>
  ...
  <Card title="Anthropic" icon="setting">
    Claude 3.5 Sonnet, Claude 3.5 Haiku, Claude 3 Opus. Models use
    `-latest` aliases (e.g. `claude-3-5-sonnet-latest`).
  </Card>
...
<Aside type="caution">
  `_flow_id` / `_flow_name` are written into session data when a flow is started by a matched
...
<Aside type="note">
  Substitution is verbatim — there's no URL-encoding pass. Since phone numbers are stored without
