...
| `{{trigger_message}}` | The incoming user message that triggered the chatbot flow (available as a session variable). |
| `{{last_message}}` | The last incoming user message received, updated each turn where user input is not empty (available as a session variable). |
...
<Aside type="caution">
  `trigger_message` and `last_message` are reserved keys. If you configure a prompt or API response mapping to store a value under these keys, it will be silently overwritten by the runner during execution.
</Aside>
