...
- **API**: pass the value via the `header_params` field on
  [Send Template Message](/whatomate/api-reference/messages/#send-template-message). It
...
- **Campaigns**: add a `header` column (always before the body parameter
  ...
  [Adding Recipients](/whatomate/features/campaigns/#adding-recipients).
...
Whatomate stores this on each template and refreshes it on every **Sync from
...
When sending via the API, provide button values using the `button_params` field. See the [API reference](/whatomate/api-reference/messages/#send-template-message) for details.
